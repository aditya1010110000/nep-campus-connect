...
//     { key: 'constraints', label: 'NEP Constraints', icon: Settings, color: 'text-gray-600' }
//   ];
...
const TimetableGenerator = () => {
  ...
  // Check if all required files are uploaded (constraints is optional)
  const areRequiredFilesUploaded = () => {
    const requiredFiles = ["students", "faculty", "rooms", "courses"];
    return requiredFiles.every((fileType) => uploadedFiles[fileType].uploaded);
  };

  const getMissingFiles = () => {
    const requiredFiles = ["students", "faculty", "rooms", "courses"];
    const missingFiles = requiredFiles.filter(
      (fileType) => !uploadedFiles[fileType].uploaded
    );
    return missingFiles.map((fileType) => {
      const section = uploadSections.find((s) => s.key === fileType);
      return section?.label || fileType;
    });
  };
  ...
  const simulateGeneration = async () => {
    // Validate required files before starting generation
    if (!areRequiredFilesUploaded()) {
      const missingFiles = getMissingFiles();
      toast({
        title: "Missing Required Files",
        description: `Please upload the following required files: ${missingFiles.join(
          ", "
        )}`,
        variant: "destructive",
      });
    ...
    setDepartmentStatuses((prev) =>
      prev.map((dept) => ({ ...dept, status: "pending" as const }))
    );
  ...
  const requiredFilesCount = uploadSections.filter(
    (section) => section.required
  ).length;
  const uploadedRequiredCount = uploadSections.filter(
    (section) => section.required && uploadedFiles[section.key].uploaded
  ).length;
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-8">
      ...
      <div className="text-center">
        ...
        <p className="text-lg text-gray-600">
          Generate optimized timetables for your institution using our
        ...
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-blue-900">
            ...
            <span className="ml-auto text-sm font-normal text-gray-600">
              {uploadedRequiredCount}/{requiredFilesCount} required files
              uploaded
            </span>
          </CardTitle>
          <CardDescription>
            Upload your institutional data files to generate optimized
            timetables
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {uploadSections.map(
              ({ key, label, icon: Icon, color, required }) => (
                <div
                  key={key}
          ...
          <div className="mt-6">
            <div className="flex justify-between items-center mb-2">
              ...
              <span className="text-sm text-gray-500">
                {uploadedRequiredCount}/{requiredFilesCount}
              </span>
            </div>
            <Progress
              value={(uploadedRequiredCount / requiredFilesCount) * 100}
              className="h-2"
            />
          </div>
        </CardContent>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-yellow-700">
                  Please upload the following required files before generating
                  timetables: {getMissingFiles().join(", ")}
                </p>
              </div>
            </div>
          </CardContent>
      ...
      <div className="text-center">
        <Button
          onClick={simulateGeneration}
          disabled={isGenerating || !areRequiredFilesUploaded()}
          className={`px-12 py-6 text-lg font-semibold rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 ${
            areRequiredFilesUploaded()
        ...
        {!areRequiredFilesUploaded() && (
          <p className="text-sm text-gray-500 mt-2">
            Upload all required files to enable timetable generation
          </p>
        )}
      ...
      <Card className="border-2 border-blue-200 bg-gradient-to-r from-blue-50 to-indigo-50">
        <CardHeader>
          ...
          <CardDescription className="text-blue-700">
            Get early access to our next-generation AI-powered timetable
            generator with advanced optimization algorithms
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
              ...
              >
                ...
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>Join Beta Program</DialogTitle>
                    <DialogDescription>
                      Be among the first to experience our advanced timetable
                      generator. We'll notify you when beta access becomes
                      available.
                    </DialogDescription>
                  </DialogHeader>
                  <div className="space-y-4">
